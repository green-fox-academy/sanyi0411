...
int main(int argc, char** argv)
{
	...
	while (true)
	{
		...
		cv::Mat grayscaled;
		cvtColor(imgOriginal, grayscaled, cv::COLOR_BGR2GRAY);
		cvtColor(grayscaled, grayscaled, cv::COLOR_GRAY2BGR);
		imgOriginal.copyTo(grayscaled, imgThresholded);

		cv::imshow("Result", grayscaled);
