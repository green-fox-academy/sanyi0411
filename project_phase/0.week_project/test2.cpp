...
void myMouseCallback(int event, int x, int y, int flags, void* userdata)
{
	...
	if (flags & cv::MouseEventFlags::EVENT_FLAG_LBUTTON) {
		cv::line(cloned, { windowColumns / 2, windowRows / 2 }, { x, y }, CV_RGB(0, 0, 0));
		...
		cv::putText(cloned, degree, { x + 20, y + 20 }, cv::FONT_HERSHEY_SIMPLEX, 1, CV_RGB(0, 0, 0), 1, cv::LINE_8, false);
		//cv::rectangle(image, { x,y }, { y, x }, CV_RGB(255, (x*y) % 255, (x + y) % 255));
	}
	cv::imshow("Ablak", cloned);
}
